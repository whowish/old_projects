...
class TC_Perl_In < Test::Unit::TestCase # :nodoc:
  def test_perl_test_cases
    ...
    opt = XmlSimple.xml_in('files/test1.xml', { 'force_array' => false, 'key_attr' => %w(name key id) })
    ...
    opt = XmlSimple.xml_in('test2.xml', {
      'force_array' => false,
      'key_attr'    => %w(name key id),
      'search_path' => %w(dir1 dir2 files/subdir)
    })
    ...
    fh = File.new('files/1_xml_in.xml')
    ...
    $stdin.reopen(File.new('files/1_xml_in.xml'))
    ...
    opt = XmlSimple.xml_in('files/srt.xml', { 'key_attr' => %w(name key id) })
    target = {
      'global' => [
        {
          'proxypswd' => 'bar',
