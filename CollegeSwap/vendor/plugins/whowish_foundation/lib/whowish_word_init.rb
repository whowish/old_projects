...
class ActionView::Base
  ...
  def self.whowish_word_for(page_id,id,*p)
    ...
    edit = "<span class=\"edit_#{word_id}\" style='"+css+"' onclick=\"$.whowish_box({title: 'WhowishWord',url:'/whowish_word/edit_box?word_id=#{word_id}&word_arg=#{js_arg.join(',')}'});var event=arguments[0];if (!event) event = window.event;event.cancelBubble = true;if (event.stopPropagation) event.stopPropagation();return false;\"><img src='/whowish_foundation_asset/edit_icon.gif'/></span>"
