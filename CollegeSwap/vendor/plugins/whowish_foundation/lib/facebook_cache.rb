class FacebookCache < ActiveRecord::Base
  ...
  def get_badge(color='')
    ...
    if is_guest
      ...
      return '<a href="'+profile_url+'" target="_new_<%=user.facebook_id%>" title="Go to '+get_possessive_adj(self).downcase+' facebook" class="facebook_profile_link"></a> <a '+style_color+' href="'+home_url+'"  title="Go to '+get_possessive_adj(self).downcase+' home page">'+name+'</a>'
    end
