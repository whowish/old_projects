module ImageHelper
  ...
  def inner_update_picture(picture_list,image_class,field_id,prefix)
      ...
      temp_images_name.each_index do |i|
        if !images_name.include?(temp_images_name[i])
          temp_images_name[i] = inner_add_image(field_id, image_class, prefix, temp_images_name[i]).original_image_path
        end
      end
      ...
      images_after_del_and_add.each do |image|
        ...
        order = images_after_del_and_add.length+1
        temp_images_name.each_index { |i|
          if temp_images_name[i] == image.original_image_path
            order = i
            break
          end
        }
