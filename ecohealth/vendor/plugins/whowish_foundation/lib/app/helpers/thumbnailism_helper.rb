module ThumbnailismHelper
  ...
  def make_thumbnail(file_path,w,h)
    ...
    logger.info { "thumbnail_path=#{thumbnail_path}" } 
  ...
  def image_obj_resize(image,w,h)
  ...
  def get_server_path_of(full_file_path)
    #logger.info {"Get get_server_path_of()"}
    #logger.info{"full_file_path=#{full_file_path}"}
    
    # it's already ok
    return full_file_path if full_file_path.match("^"+RAILS_ROOT+"/")
    
    full_file_path = "/"+full_file_path if !full_file_path.match("^/") 
    ...
    #logger.info{"Return full_file_path=#{full_file_path}"}
