module FacebookHelper
  ...
  def facebook_connect
    ...
    if params[:code]
      ...
      begin
        ...
        logger.info{ "Response from getting oauth_token = " + response.body[0..200] }
  ...
  def get_facebook_info(id,force_update=false)
    ...
    logger.debug { "profile=#{profile}, force_update=#{force_update}" }
  ...
  def get_friends(facebook_id,force_update=false,get_remote=true)
    ...
    if get_remote and (!friend or force_update)
      ...
      begin
        ...
        logger.info {"Result=#{result_data[0..100]}" }
    ...
    logger.debug { "Exit with friends=#{friend.friends[0..100]}" }
    ...
    def query(q)
      ...
      while retry_count < 3
        ...
        begin
          ...
          logger.info {"Exit query() with response.body=#{response.body[0..100]}"}
          ...
          FacebookLog.create(:path=>path,:sent=>"",:received=>response.body[0..1000])
  ...
  def post_data(method,data,user_id=facebook_id)
      ...
      FacebookLog.create(:path=>url.path,:sent=>nvp,:received=>response.body[0..1000])
      return response.body
  end
  ...
  def update_friend_records(facebook_id=self.facebook_id)
      return if is_guest
      
      begin 
        data = ActiveSupport::JSON.decode(get_data("friends",facebook_id))["data"]
        ...
        logger.error {"Error reading friend: #{e.to_s_with_trace}"}
      end
  end
