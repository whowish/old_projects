...
#          next if [:controller,:action].include?(k)
#          q_string.push("#{CGI.escape(k.to_s)}=#{CGI.escape(v.to_s)}")
#        }
#        
#        
#        q_string = "?#{q_string.join("&")}"
#        q_string = "" if q_string=="?"
  ...
  def get_friends(facebook_id,force_update=false,get_remote=true)
    ...
    if get_remote and (!friend or force_update)
      ...
      begin
        ...
        friend.friend_count = data.length
    ...
    def get_data(method,user_id=facebook_id)
      ...
      logger.debug {"Exit get_data() with response.body=#{response.body}"}
    ...
    def post_data(method,data,user_id=facebook_id)
      ...
      logger.debug {"Exit post_data() with response.body=#{response.body}"}
      return response.body
  end
