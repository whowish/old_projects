...
class TC_Perl_Out < Test::Unit::TestCase # :nodoc:
  def test_perl_test_cases
    ...
    assert_equal('<opt three="..." two="II" one="1" />', xml.strip)
    ...
    assert(xml =~ %r{
      <(\w+)\s*>
      \s*<Turkey\s+capital="Istanbul"\s*/>
      \s*<France\s+capital="Paris"\s*/>
      \s*<England\s+capital="London"\s*/>
      \s*</\1\s*>}sx)
    ...
    assert(File.exist?(test_file))
    x = File.readlines(test_file).to_s
    assert_equal(xml, File.readlines(test_file).to_s)
