require File.expand_path("../validator_function_unit", __FILE__)
require File.expand_path("../validator_helper", __FILE__)
      ...
      def validate(all_params)
        ...
        @__fields_to_validate.each_pair { |key,list|
          ...
          list.each { |func_with_msg|
            ...
            result = func_with_msg[:f].proc.curry[all_params[key]] 
            
            if result.kind_of?(Proc)
              result = result.curry[all_params] 
            end
